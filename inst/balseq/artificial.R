...
grad_pp <- gauss2d(X_pp[,1],X_pp[,2],0.5,0.5,1,-0.3,1,15) 
grad_ns <- gauss2d(X_ns[,1],X_ns[,2],0.5,0.5,1,-0.3,1,15) 
...
# pik 
# pik_tmp_uneq <- inclusionprobabilities(rgamma(N,1,1/5),n)
# while(any(pik_tmp_uneq > 1 - 1e-7 | pik_tmp_uneq < 1e-7)){
#   pik_tmp_uneq <- inclusionprobabilities(rgamma(N,1,1/5),n)  
# }
pik_tmp_uneq_pp <- inclusionprobabilities(grad_pp,n)
pik_tmp_uneq_ns <- inclusionprobabilities(grad_ns,n)
# while(any(pik_tmp_uneq > 1 - 1e-7 | pik_tmp_uneq < 1e-7)){
#   pik_tmp_uneq <- inclusionprobabilities(gradient,n)
# }

pik_pp <- list(eq = rep(n/N,N),
               uneq = pik_tmp_uneq_pp)

pik_ns <- list(eq = rep(n/N,N),
               uneq = pik_tmp_uneq_ns)
...
Xaux_ns <- list(eq = cbind(pik_ns$eq,Xaux_tmp),
                uneq = cbind(pik_ns$uneq,Xaux_tmp))
...
# y_tmp <- list(y1 = Xaux_tmp%*%beta + rnorm(N,0,20),
              # y2 = log(Xaux_tmp%*%beta) + rnorm(N,0,2))

beta <- c(1,1,1,1,1)
y_pp <- list(y1 = grad_pp + Xaux_tmp%*%beta + rnorm(N,0,0.1))
             # y2 = grad_pp + 0.3*X4 + X3 + exp(-X5) + rnorm(N,0.1))
y_ns <- list(y1 = grad_ns + Xaux_tmp%*%beta + rnorm(N,0,0.1))
             # y2 = grad_ns + 0.3*X4 + X3 + exp(-X5) + rnorm(N,0.1))




pairs(cbind(Xaux_tmp,y_pp$y1,pik_pp$uneq))
pairs(cbind(Xaux_tmp,y_ns$y1,pik_ns$uneq))

ggplot() +
  geom_point(data = data.frame(x = X_ns[,1],y = X_ns[,2],z = pik_ns$uneq),aes(x = x, y = y,size = z),shape = 1)
# ggplot() +
  # geom_point(data = data.frame(x = X_ns[,1],y = X_ns[,2],z = y_ns$y2),aes(x = x, y = y,color = z,size = z))
ggplot() +
  geom_point(data = data.frame(x = X_pp[,1],y = X_pp[,2],z = pik_pp$uneq ),aes(x = x, y = y,size = z),shape = 1)
# ggplot() +
  # geom_point(data = data.frame(x = X_pp[,1],y = X_pp[,2],z = y_pp$y2 ),aes(x = x, y = y,color = z,size = z))
...
# sim_design(1,f_eq,Xaux$eq,Xspread$ns,D$ns,pik$eq,y$eq)
# sim_design(1,f_uneq,Xaux$uneq,Xspread$ns,D$ns,pik$uneq,y$uneq)
...
print(Sys.time() -  start)
start <- Sys.time()
res_ns_eq = parallel::parLapply(cl = cl,
                                X = 1:SIM,
                                fun =  sim_design,
                                f = f_eq,
                                Xaux = Xaux_ns$eq,
...
print(Sys.time() -  start)
start <- Sys.time()
res_ns_uneq = parallel::parLapply(cl = cl,
                                  X = 1:SIM,
                                  fun =  sim_design,
                                  f = f_uneq,
                                  Xaux = Xaux_ns$uneq,
...
saveRDS(res,file = "C:/Users/jauslinr/switchdrive/StratifiedSampling/StratifiedSampling/inst/balseq/res_artificial_25052022_2.rds")
...
# 
# ns_dev <- rep(list(0),2)
# names(ns_dev) <- c("eq","uneq")
# 
# pp_dev$eq <- pp_eq_dev[[1]]
# pp_dev$uneq <- pp_uneq_dev[[1]]
# ns_dev$eq <- ns_eq_dev[[1]]
# ns_dev$uneq <- ns_uneq_dev[[1]]
# 
# 
# for(i in 2:SIM){
#   pp_dev$eq  <- pp_dev$eq + pp_eq_dev[[i]]/SIM
#   pp_dev$uneq  <- pp_dev$uneq + pp_uneq_dev[[i]]/SIM
#   ns_dev$eq  <- ns_dev$eq + ns_eq_dev[[i]]/SIM
#   ns_dev$uneq  <- ns_dev$uneq + ns_uneq_dev[[i]]/SIM
# }
# 
...
# dev
# 
# rownames(dev) <- rep(c(f_eq_name,f_uneq_name),times = 2)
# colnames(dev) <- c("Inclusion Probabilities","$X_1$","$X_2$","$X_3$","$X_4$","$X_5$")
# 
# 
# ############# SPREAD AND VARIANCE
# 
# ns_eq_spread <- colMeans(do.call(rbind,lapply(ns_eq_spread,unlist)))
# ns_uneq_spread <- colMeans(do.call(rbind,lapply(ns_uneq_spread,unlist)))
...
# 
# ns_eq_v <- colMeans(do.call(rbind,lapply(ns_eq_v,unlist)))
# ns_uneq_v <- colMeans(do.call(rbind,lapply(ns_uneq_v,unlist)))
# pp_eq_v <- colMeans(do.call(rbind,lapply(pp_eq_v,unlist)))
# pp_uneq_v <- colMeans(do.call(rbind,lapply(pp_uneq_v,unlist)))
# 
# ns_eq_vEst <- colMeans(do.call(rbind,lapply(ns_eq_vEst,unlist)))
# ns_uneq_vEst <- colMeans(do.call(rbind,lapply(ns_uneq_vEst,unlist)))
# pp_eq_vEst <- colMeans(do.call(rbind,lapply(pp_eq_vEst,unlist)))
# pp_uneq_vEst <- colMeans(do.call(rbind,lapply(pp_uneq_vEst,unlist)))
# 
# 
# o_eq_sb <- grepl(pattern= 'sb',names(ns_eq_spread))
# o_uneq_sb <- grepl(pattern= 'sb',names(ns_uneq_spread))
# o_eq_IB <- grepl(pattern= 'IB',names(ns_eq_spread))
# o_uneq_IB <- grepl(pattern= 'IB',names(ns_uneq_spread))
# 
# o_eq_y1 <- grepl(pattern= 'y1',names(ns_eq_v))
# o_uneq_y1 <- grepl(pattern= 'y1',names(ns_uneq_v))
...
# 
# 
# ns_eq_vEst <- ns_eq_vEst/ns_eq_v*100
# ns_uneq_vEst <- ns_uneq_vEst/ns_uneq_v*100
# pp_eq_vEst <- pp_eq_vEst/pp_eq_v*100
# pp_uneq_vEst <- pp_uneq_vEst/pp_uneq_v*100
# 
# 
# 
# ns_v_y1 <- c(ns_eq_v[o_eq_y1]/ns_eq_v[o_eq_y1][which(grepl('srswor',names(ns_eq_v[o_eq_y1])))]*100,
#                  ns_uneq_v[o_uneq_y1]/ns_uneq_v[o_eq_y1][which(grepl('cps',names(ns_uneq_v[o_eq_y1])))]*100)
# # ns_v_y2 <- c(ns_eq_v[o_eq_y2]/ns_eq_v[o_eq_y2][which(grepl('srswor',names(ns_eq_v[o_eq_y2])))]*100,
# #                  ns_uneq_v[o_uneq_y2]/ns_uneq_v[o_eq_y2][which(grepl('cps',names(ns_uneq_v[o_eq_y2])))]*100)
...
# # pp_v_y2 <- c(pp_eq_v[o_eq_y2]/pp_eq_v[o_eq_y2][which(grepl('srswor',names(pp_eq_v[o_eq_y2])))]*100,
# #                  pp_uneq_v[o_uneq_y2]/pp_uneq_v[o_eq_y2][which(grepl('cps',names(pp_uneq_v[o_eq_y2])))]*100)
# 
# # ns_v <- cbind(ns_v_y1,ns_v_y2)
# ns_v <- cbind(ns_v_y1)
# pp_v <- cbind(pp_v_y1)
# # pp_v <- cbind(pp_v_y1,pp_v_y2)
# 
# 
# ns_vEst_y1 <- c(ns_eq_vEst[o_eq_y1],ns_uneq_vEst[o_uneq_y1])
# # ns_vEst_y2 <- c(ns_eq_vEst[o_eq_y2],ns_uneq_vEst[o_uneq_y2])
# pp_vEst_y1 <- c(pp_eq_vEst[o_eq_y1],pp_uneq_vEst[o_uneq_y1])
# # pp_vEst_y2 <- c(pp_eq_vEst[o_eq_y2],pp_uneq_vEst[o_uneq_y2])
# 
# 
...
# ns_vEst <- cbind(ns_vEst_y1)
# pp_vEst <- cbind(pp_vEst_y1)
# # pp_vEst <- cbind(pp_vEst_y1,pp_vEst_y2)
# 
# 
# ns_spread_sb <- c(ns_eq_spread[o_eq_sb],ns_uneq_spread[o_uneq_sb])
# ns_spread_IB <- c(ns_eq_spread[o_eq_IB],ns_uneq_spread[o_uneq_IB])
# pp_spread_sb <- c(pp_eq_spread[o_eq_sb],pp_uneq_spread[o_uneq_sb])
# pp_spread_IB <- c(pp_eq_spread[o_eq_IB],pp_uneq_spread[o_uneq_IB])
# 
# 
# ns_spread <- cbind(ns_spread_sb,ns_spread_IB)
# pp_spread <- cbind(pp_spread_sb,pp_spread_IB)
# 
# 
# ns <- cbind(ns_v,ns_vEst,ns_spread)
# pp <- cbind(pp_v,pp_vEst,pp_spread)
# 
# tab <- rbind(ns,pp)
# tab
# 
# rownames(tab) <- rep(c(f_eq_name,f_uneq_name),times = 2)
